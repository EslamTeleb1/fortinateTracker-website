...
const listItems=async()=>{
    // Get getDailyShop
    const listItems = await fortniteAPI.listItems(lang='en');

    return listItems
}

listItems().then(function(result){
    ...
    //     console.log(types[j])
        ...
        objJson=result.items[types[Math.floor(Math.random() * 16)]];
        var pagination_container = document.getElementById("pagination-container");
        for(let i=0;i<=objJson.length-1;i++)
        {
            var border= objJson[i].rarity==='uncommon'?'handmade': objJson[i].rarity;
            ...
            nwHtml=html.replace('%id%',  objJson[i].id);
            // src of the image
            if(objJson[i].images)
            nwHtml=nwHtml.replace('%img%', objJson[i].images.icon);
            //change border style
             nwHtml=nwHtml.replace('%rarity%',border)
            nwHtml=nwHtml.replace('%name%', objJson[i].name)
            nwHtml=nwHtml.replace('%title%', objJson[i].name)
            nwHtml=nwHtml.replace('%price%', objJson[i].price)
            if(objJson[i].interest)
            nwHtml=nwHtml.replace('%star%', objJson[i].interest.toFixed(2))
   ...
   // }
    ...
    document.getElementById('filter-category-type').addEventListener("change", ()=>{
        ...
        var selectedOption=document.getElementById('filter-category-type').value;

        
            objJson=result.items[selectedOption];
       
    }); 
    
    document.getElementById('filter-category-rarity').addEventListener("change", ()=>{

        var selectedOption=document.getElementById('filter-category-rarity').value;
...
},{"fortnite-api-io":3}],3:[function(require,module,exports){
/**
 * fortnite-api-io
...
},{"./lib/fortnite-api":5}],4:[function(require,module,exports){
/**
 * fortnite-api-io
 * Module to make API calls to fortniteapi.io
 *
...
},{"./endpoints":4,"./util":6,"_process":1,"node-fetch":7}],6:[function(require,module,exports){
/**
 * fortnite-api-io
 * Module to make API calls to fortniteapi.io
 *
